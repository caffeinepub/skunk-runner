...
function createSoundEngine() {
  ...
  function playTone(frequency, type, duration, volume = 0.18, freqEnd, delay = 0) {
    try {
      const ac = getCtx();
  ...
  return {
    ...
    collectSun() {
      ...
      playTone(1100, "sine", 0.08, 0.12, void 0, 0.06);
    },
    hit() {
    ...
    death() {
      for (let i = 0; i < 4; i++) {
        playTone(400 - i * 80, "square", 0.18, 0.18, void 0, i * 0.18);
      }
    },
    stomp() {
    ...
    spliffLit() {
      ...
      playTone(1050, "triangle", 0.1, 0.1, void 0, 0.34);
    },
    start() {
      playTone(440, "square", 0.1, 0.15, void 0, 0);
      playTone(550, "square", 0.1, 0.15, void 0, 0.1);
      playTone(660, "square", 0.15, 0.18, void 0, 0.2);
      playTone(880, "square", 0.2, 0.2, void 0, 0.32);
    }
  };
}
...
function drawCannabisLeafShape(ctx, cx, cy, r, fillColor, strokeColor) {
  ctx.save();
  ctx.translate(cx, cy);
  const fingers = [
    [0, 1, 0.11, 0],
    // centre — tallest
    [0.62, 0.86, 0.1, 0.06],
    // inner right — bigger gap
    [-0.62, 0.86, 0.1, -0.06],
    // inner left  — bigger gap
    [1.15, 0.74, 0.09, 0.08],
    // mid right — wider spread
    [-1.15, 0.74, 0.09, -0.08],
    // mid left  — wider spread
    [1.65, 0.58, 0.08, 0.42],
    // outer right — wider + drooping curl
    [-1.65, 0.58, 0.08, -0.42]
    // outer left  — wider + drooping curl
  ];
  function drawFinger(angle, lenFactor, hwFactor, curl = 0) {
    const L = r * lenFactor;
    const W = r * hwFactor;
    const TEETH = 5;
    ctx.save();
    ...
    function profile(t) {
      if (t < 0.2) return W * (t / 0.2) * 0.55;
      if (t < 0.4) return W * (0.55 + 0.45 * ((t - 0.2) / 0.2));
      return W * (1 - (t - 0.4) / 0.6);
    }
    const pts = [];
    const steps = TEETH * 6;
    for (let i = 0; i <= steps; i++) {
      ...
      const bw = profile(t);
      const y = -L * t;
      const toothAmp = t > 0.12 && t < 0.95 ? bw * 0.28 * Math.max(0, Math.sin(t * TEETH * Math.PI)) : 0;
...
function drawMickeyGlove(ctx, cx, cy, r, facingRight) {
  ctx.save();
  ctx.translate(cx, cy);
...
function drawBloodshotEyes(ctx, lx, ly, rx, ry, eyeR) {
  const eyes = [
    { cx: lx, cy: ly },
    { cx: rx, cy: ry }
  ];
  for (const eye of eyes) {
    ...
    ctx.fill();
    const veinAngles = [0.3, 1, 1.9, 3.4, 4.7, 5.4];
...
function drawLeafMouth(ctx, cx, my, size) {
  ctx.strokeStyle = GB.darkest;
  ctx.lineWidth = 1.4;
  ctx.beginPath();
...
function drawSpliff(ctx, cx, my, size, alpha) {
  ctx.save();
  ctx.globalAlpha = alpha;
  const angle = -0.5;
  const jointLen = size * 3.2;
  const jointW = size * 0.22;
  const mouthW = jointW * 0.3;
  const emberW = jointW * 1.1;
...
function drawHudSpliffIcon(ctx, x, y) {
  const len = 14;
  const mW = 1.5;
  const eW = 4.5;
  ctx.save();
...
function drawCurledLeafFoot(ctx, cx, topY, r, facingRight) {
  ctx.save();
  ctx.translate(cx, topY);
  const footFingers = [
    [Math.PI / 2, 1, 0.28, 0],
    // main toe — points right (forward)
    [Math.PI / 2 - 0.55, 0.72, 0.2, -0.15],
    // upper curl — angled slightly up
    [Math.PI / 2 + 0.55, 0.72, 0.2, 0.15]
    // lower curl — angled slightly down
  ];
  for (const [angle, lenFactor, hwFactor, curl] of footFingers) {
    ...
    for (let i = 0; i <= steps; i++) {
      const t = i / steps;
      const bw = t < 0.25 ? W * (t / 0.25) * 0.5 : t < 0.5 ? W * (0.5 + 0.5 * ((t - 0.25) / 0.25)) : W * (1 - (t - 0.5) / 0.5);
      const toothAmp = t > 0.15 && t < 0.9 ? bw * 0.22 * Math.max(0, Math.sin(t * TEETH * Math.PI)) : 0;
      ctx.lineTo(bw + toothAmp + 1, -L * t);
    }
    ctx.lineTo(0, -L - 1.2);
...
function drawPlayer(ctx, p, screenX, spliffAlpha = 0) {
  ...
  if (p.ducking) {
    ...
    const swayAngle = p.onGround && isMoving ? p.animFrame % 2 === 0 ? -0.08 : 0.08 : 0;
    ctx.save();
    ...
    const thighLen = 13;
    const shinLen = 13;
    const walkPhase = p.animFrame / 4 * Math.PI * 2;
    const isWalking = Math.abs(p.vx) > 0.3 && p.onGround;
    const thighSwing = isWalking ? 0.45 : 0;
    const lThighAngle = Math.sin(walkPhase) * thighSwing;
    const rThighAngle = Math.sin(walkPhase + Math.PI) * thighSwing;
    const lShinAngle = isWalking ? Math.max(0, Math.sin(walkPhase - 0.4)) * 0.5 : 0;
    const rShinAngle = isWalking ? Math.max(0, Math.sin(walkPhase + Math.PI - 0.4)) * 0.5 : 0;
    ...
    ctx.fill();
    drawCurledLeafFoot(ctx, lFootX, lFootY, 9);
    ...
    ctx.fill();
    drawCurledLeafFoot(ctx, rFootX, rFootY, 9);
  }
  ctx.restore();
}
function drawAerosolEnemy(ctx, e, screenX) {
...
function drawPolicemanEnemy(ctx, e, screenX) {
  const x = Math.floor(screenX);
  const y = Math.floor(e.y);
  const w = e.width;
  const bodyH = 22;
...
function drawPlantPot(ctx, pot, screenX) {
  const x = Math.floor(screenX);
...
function drawPlatform(ctx, p, worldOffset) {
...
function drawRouteArrow(ctx, sx) {
  const ax = sx;
...
function drawLeafCloudCutout(ctx, cx, cy, r) {
  const fingers = [
    [0, 1, 0.13, 0],
    [0.62, 0.86, 0.12, 0.06],
    [-0.62, 0.86, 0.12, -0.06],
  ...
  for (const [angle, lenFactor, hwFactor, curl] of fingers) {
    ...
    for (let i = 0; i <= steps; i++) {
      const t = i / steps;
      const bw = t < 0.2 ? W * (t / 0.2) * 0.55 : t < 0.4 ? W * (0.55 + 0.45 * ((t - 0.2) / 0.2)) : W * (1 - (t - 0.4) / 0.6);
      const toothAmp = t > 0.12 && t < 0.95 ? bw * 0.25 * Math.max(0, Math.sin(t * TEETH * Math.PI)) : 0;
      ctx.lineTo(bw + toothAmp, -L * t);
    }
    ctx.lineTo(0, -L);
    for (let i = steps; i >= 0; i--) {
...
function drawLeafCloud(ctx, cx, cy, r) {
  const pad = r * 2.8;
  ...
  grad.addColorStop(0, "#c8eebc");
  grad.addColorStop(0.55, "#a8d898");
  grad.addColorStop(1, "#5aab52");
  oc.fillStyle = grad;
  const blobs = [
    [0, 0, r * 1.1],
...
function drawBackground(ctx, worldOffset) {
  ctx.fillStyle = GB.lightest;
  ...
  const cloudData = [
    { i: 0, baseY: 38, scale: 1, spacing: 115 },
  ...
  for (const { i, baseY, scale, spacing } of cloudData) {
    const cx2 = (i * spacing - cloudOffset * (0.08 + i * 0.01) + 60) % (CANVAS_W + 200) - 100;
...
function drawHUD(ctx, gs) {
  ...
  const progress = gs.coinsCollected % SPLIFF_COIN_MILESTONE / SPLIFF_COIN_MILESTONE;
  const barW = 100;
  const barH = 8;
  const barX = 6;
...
function drawWaterDrop(ctx, x, y, offset) {
...
function drawSunRay(ctx, x, y, offset) {
  const cy = y + Math.sin(offset * 0.05) * 3;
  const cx = x + 6;
  ctx.fillStyle = GB.darkest;
  for (let i = 0; i < 8; i++) {
...
function pseudoRandom(seed) {
  const s = (seed * 1664525 + 1013904223 & 4294967295) >>> 0;
  return s / 4294967295;
}
function trySpawnObstacle(gs, worldX, seed, difficultyLevel) {
...
function spawnEnemy(gs, worldX, seed, platLeft, platRight, forceType) {
  const type = pseudoRandom(seed + 99) > 0.5 ? "police" : "aerosol";
  gs.enemies.push({
    x: worldX,
    y: GROUND_Y - 36,
    width: 22,
...
function spawnNewChunk(gs, chunkX) {
  const seed = gs.platformSeed++;
  const difficultyLevel = Math.min(Math.floor(gs.distance / 600) + 1, 10);
  const routeType = pseudoRandom(seed + 50) > 0.4 ? pseudoRandom(seed + 51) > 0.5 ? 2 : 1 : 0;
  if (routeType >= 1) {
    ...
    for (let pi = 0; pi < numHighPlatforms; pi++) {
      ...
      const platformY = GROUND_Y - 70 - Math.floor(pseudoRandom(seed + 62 + pi) * 60);
      const offsetX = pi * (pw + 20 + Math.floor(pseudoRandom(seed + 63 + pi) * 30));
      gs.platforms.push({
        x: chunkX + offsetX,
        y: platformY,
        width: pw,
      ...
      if (pseudoRandom(seed + 68 + pi) < 0.33) {
        const etype = pseudoRandom(seed + 69 + pi) > 0.5 ? "police" : "aerosol";
        gs.enemies.push({
...
function spawnGuaranteedBatch(gs, worldX) {
  const seed = gs.platformSeed++;
  const difficultyLevel = Math.min(Math.floor(gs.distance / 600) + 1, 10);
  trySpawnObstacle(gs, worldX, seed + 1, difficultyLevel);
  const framesSinceEnemy = gs.frameCount - gs.lastEnemyFrame;
  if (framesSinceEnemy >= MAX_ENEMY_INTERVAL_FRAMES || pseudoRandom(seed + 5) < 0.33)
...
const ROUTE_ARROW_WORLD_POSITIONS = [
  600,
  1200,
  1900,
  2700,
...
function createInitialGameState(hiScore, gameSpeed = 1) {
  const gs = {
    phase: "playing",
    score: 0,
    hiScore,
    ...
    enemies: [],
    platforms: [
      { x: -100, y: GROUND_Y, width: 2e4, height: 40, isGround: true },
...
function updateGame(gs, keys, _dt, _frameCount) {
  ...
  for (const plat of gs.platforms) {
    ...
    if (overlapX && p.vy >= 0 && pBottom >= plat.y && pBottom <= plat.y + plat.height + p.vy + 4) {
      p.y = plat.y - p.height;
      p.vy = 0;
      p.onGround = true;
    } else if (!plat.isGround && overlapX && p.vy < 0 && pTop <= plat.y + plat.height && pTop >= plat.y) {
  ...
  for (const obs of gs.obstacles) {
    ...
    if (overlapX && overlapY) {
      if (p.vy >= 0 && pBottom >= obs.y && pBottom <= obs.y + 12 + Math.abs(p.vy)) {
        p.y = obs.y - p.height;
        p.vy = 0;
        p.onGround = true;
      } else if (p.vy < 0 && pTop <= obs.y + obs.height && pTop >= obs.y + obs.height - 8) {
  ...
  for (const e of gs.enemies) {
    ...
    const hitX = Math.abs(p.x + p.width / 2 - (eScreenX + ne.width / 2)) < p.width / 2 + ne.width / 2 - 4;
    const stompCheck = hitX && p.vy > 0 && p.y + p.height >= ne.y && p.y + p.height <= ne.y + ne.height * 0.5 && !wasOnGround;
    if (stompCheck) {
      ne.stomped = true;
      ne.squishTimer = 18;
      p.vy = -7;
    ...
    if (p.invincibleTimer <= 0) {
      const hitY = Math.abs(p.y + p.height / 2 - (ne.y + ne.height / 2)) < p.height / 2 + ne.height / 2 - 4;
      if (hitX && hitY) {
        p.lives -= 1;
        p.invincibleTimer = 120;
        if (p.lives <= 0) {
  ...
  for (const pot of tempPots) {
    ...
    if (np.gardenerVisible && np.gardenerTimer > 5) {
      ...
      const stompG = hitX2 && p.vy > 0 && pBottom >= gardenTop && pBottom <= gardenTop + 14 && !wasOnGround;
      if (stompG) {
        np.stomped = true;
        np.squishTimer = 18;
        p.vy = -7;
  ...
  const liveEnemiesOnScreen = tempGs.enemies.filter(
    (e) => !e.stomped && e.x - newWorldOffset > -50 && e.x - newWorldOffset < CANVAS_W + 100
  ).length;
  if (currentFrame - tempGs.lastEnemyFrame >= MAX_ENEMY_INTERVAL_FRAMES && liveEnemiesOnScreen < 2) {
  ...
  const gsMultiplier = gs.distance < 50 ? gs.scrollSpeed / SCROLL_BASE : Math.max(
    0.5,
    gs.scrollSpeed / (SCROLL_BASE + Math.min(gs.distance / 3e3, difficultyLevel * 0.3))
  );
  const newScrollSpeed = Math.min(
    SCROLL_BASE * gsMultiplier + gs.distance / 3e3,
...
function SkunkRunner() {
  ...
  const hiScoreRef = reactExports.useRef(0);
  const touchRef = reactExports.useRef({ left: false, jump: false, right: false });
  const soundRef = reactExports.useRef(null);
  const soundOnRef = reactExports.useRef(true);
  const gameSpeedRef = reactExports.useRef(1);
  const phaseRef = reactExports.useRef("start");
  const settingsReturnPhaseRef = reactExports.useRef("start");
  reactExports.useEffect(() => {
  ...
  const [phase, setPhaseState] = reactExports.useState("start");
  const setPhase = reactExports.useCallback(
    (p) => {
      phaseRef.current = p;
      setPhaseState(p);
    },
  ...
  const [hiScore, setHiScore] = reactExports.useState(0);
  const [_lives, setLives] = reactExports.useState(3);
  const [spliffsSmoked, setSpliffsSmoked] = reactExports.useState(0);
  const [enemiesKilled, setEnemiesKilled] = reactExports.useState(0);
  ...
  const ensureSound = reactExports.useCallback(() => {
  ...
  const goToMainMenu = reactExports.useCallback(() => {
    if (gsRef.current) gsRef.current.phase = "paused";
    gsRef.current = null;
    setPhase("start");
  }, [setPhase]);
  const openSettingsFromPause = reactExports.useCallback(() => {
    if (gsRef.current) gsRef.current.phase = "paused";
  ...
  reactExports.useEffect(() => {
    ...
    function loop(time) {
      ...
      if (((_a2 = gsRef.current) == null ? void 0 : _a2.phase) === "gameover" && gsRef.current.deathFadeTimer > 0) {
        gsRef.current = {
          ...gsRef.current,
  ...
  function drawStartScreen(ctx, suppressPrompt = false) {
    const now2 = Date.now();
    ctx.fillStyle = GB.lightest;
    ctx.fillRect(0, 0, CANVAS_W, CANVAS_H);
    ctx.fillStyle = GB.light;
    for (let i = 0; i < 6; i++) ctx.fillRect(i * 80, 0, 40, CANVAS_H);
    const cloudT = now2 * 8e-5;
    const introClouds = [
      { baseX: 60, cy: 32, r: 28, speed: 1 },
    ...
    const PIXEL_FONT2 = "'Press Start 2P', 'Courier New', monospace";
    const titleY1 = 52;
    const titleY2 = 86;
    const shadowOffsets = [
      [3, 3],
    ...
    const px = CANVAS_W / 2;
    const leafR = 52;
    const danceFrame = Math.floor(now2 / 200) % 6;
    const bodyBounce = danceFrame === 4 ? -3 : danceFrame === 5 ? 3 : 0;
    const bodyLean = danceFrame === 1 ? 0.15 : danceFrame === 3 ? -0.15 : 0;
    const leftArmUp = danceFrame === 0 ? -15 : danceFrame === 4 ? -12 : danceFrame === 5 ? 5 : 2;
    const rightArmUp = danceFrame === 2 ? -15 : danceFrame === 4 ? -12 : danceFrame === 5 ? 5 : 2;
    ...
    ctx.fillStyle = GB.dark;
    ctx.fillText(
      `© ${(/* @__PURE__ */ new Date()).getFullYear()} SKUNK RUNNER  |  BUILT WITH CAFFEINE.AI`,
      CANVAS_W / 2,
      308
    );
  }
  function renderFrame(ctx, gs) {
    ...
    const spliffAlpha = gs.spliffTimer > 0 ? Math.min(1, gs.spliffTimer / (SPLIFF_DURATION_FRAMES * 0.2)) : 0;
  ...
  reactExports.useEffect(() => {
    function onKeyDown(e) {
      ...
      if ((e.code === "Space" || e.code === "Enter") && (phase === "start" || phase === "gameover"))
        startGame();
    }
    function onKeyUp(e) {
  ...
  const jumpBtn = /* @__PURE__ */ jsxRuntimeExports.jsx(
    "button",
    {
      type: "button",
  ...
  const leftBtn = /* @__PURE__ */ jsxRuntimeExports.jsx(
    "button",
    {
      type: "button",
  ...
  const rightBtn = /* @__PURE__ */ jsxRuntimeExports.jsx(
    "button",
    {
      type: "button",
      "data-ocid": "game.secondary_button",
  ...
  const controlLeft = settings.layout === "A" ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", gap: 8, alignItems: "center" }, children: [
    jumpBtn,
    leftBtn
  ] }) : settings.layout === "B" ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", gap: 8, alignItems: "center" }, children: [
    leftBtn,
    rightBtn
  ] }) : (
    // Layout C: nothing on left
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { display: "flex", gap: 8, alignItems: "center" } })
  );
  const controlRight = settings.layout === "A" ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { display: "flex", gap: 8, alignItems: "center" }, children: rightBtn }) : settings.layout === "B" ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { display: "flex", gap: 8, alignItems: "center" }, children: jumpBtn }) : (
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "div",
    {
      ...
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "div",
          {
            ...
            children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "div",
                {
                  ...
                  children: [
                    ...
                    phase === "paused" && /* @__PURE__ */ jsxRuntimeExports.jsxs(
                      "div",
                      {
                    ...
                    phase === "settings" && /* @__PURE__ */ jsxRuntimeExports.jsx(
                      "div",
                      {
                        ...
                        children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
                          "div",
                          {
                            ...
                            children: [
                              ...
                              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { marginBottom: 16, textAlign: "left" }, children: [
                                /* @__PURE__ */ jsxRuntimeExports.jsx(
                                  "div",
                                  {
                                    style: {
                                    ...
                                    children: [
                                      /* @__PURE__ */ jsxRuntimeExports.jsx(
                                        "button",
                                        {
                                          ...
                                          style: settings.layout === "A" ? activeBtnStyle : inactiveBtnStyle,
                                      ...
                                      /* @__PURE__ */ jsxRuntimeExports.jsx(
                                        "button",
                                        {
                                          ...
                                          style: settings.layout === "B" ? activeBtnStyle : inactiveBtnStyle,
                                      ...
                                      /* @__PURE__ */ jsxRuntimeExports.jsx(
                                        "button",
                                        {
                                          ...
                                          style: settings.layout === "C" ? activeBtnStyle : inactiveBtnStyle,
                                    ...
                                    children: [
                                      /* @__PURE__ */ jsxRuntimeExports.jsx(
                                        "button",
                                        {
                                          ...
                                          style: settings.buttonSize === "small" ? activeBtnStyle : inactiveBtnStyle,
                                          children: "S"
                                        }
                                      ),
                                      /* @__PURE__ */ jsxRuntimeExports.jsx(
                                        "button",
                                        {
                                          ...
                                          style: settings.buttonSize === "medium" ? activeBtnStyle : inactiveBtnStyle,
                                          children: "M"
                                        }
                                      ),
                                      /* @__PURE__ */ jsxRuntimeExports.jsx(
                                        "button",
                                        {
                                          ...
                                          style: settings.buttonSize === "large" ? activeBtnStyle : inactiveBtnStyle,
                                          children: "L"
                                        }
                                      )
                                    ]
                              ...
                              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { marginBottom: 16, textAlign: "left" }, children: [
                                ...
                                /* @__PURE__ */ jsxRuntimeExports.jsxs(
                                  "div",
                                  {
                                    ...
                                    children: [
                                      settings.gameSpeed.toFixed(1),
                                      "x"
                                    ]
                                  }
                                )
                              ] }),
                              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { marginBottom: 20, textAlign: "left" }, children: [
                                /* @__PURE__ */ jsxRuntimeExports.jsx(
                                  "div",
                                  {
                                    style: {
                                  ...
                                  "div",
                                  {
                                    ...
                                    children: /* @__PURE__ */ jsxRuntimeExports.jsx(
                                      "button",
                                      {
                                        ...
                                        style: settings.soundOn ? activeBtnStyle : inactiveBtnStyle,
                                        "aria-label": settings.soundOn ? "Sound on" : "Sound off",
                                        children: settings.soundOn ? "♪ ON" : "✕ OFF"
                                      }
                                    )
                                  }
                                )
              ...
              (phase === "playing" || phase === "paused") && /* @__PURE__ */ jsxRuntimeExports.jsx(
                "button",
                {
                  type: "button",
              ...
              (phase === "start" || phase === "settings") && /* @__PURE__ */ jsxRuntimeExports.jsx(
                "button",
                {
                  type: "button",
              ...
              (phase === "playing" || phase === "paused") && /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "div",
                {
                  style: {
                    background: GB.dark,
        ...
        phase === "gameover" && /* @__PURE__ */ jsxRuntimeExports.jsx(
          "div",
          {
            ...
            children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "div",
              {
                ...
                children: [
                  ...
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { color: GB.lightest, fontSize: 14, marginBottom: 4 }, children: [
                    "SCORE: ",
                    String(score).padStart(6, "0")
                  ] }),
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { color: GB.light, fontSize: 12, marginBottom: 8 }, children: [
                    "BEST: ",
                    String(hiScore).padStart(6, "0")
                  ] }),
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { color: GB.lightest, fontSize: 11, marginBottom: 4 }, children: [
                    "SPLIFFS SMOKED: ",
                    String(spliffsSmoked).padStart(3, "0")
                  ] }),
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { color: GB.lightest, fontSize: 11, marginBottom: 20 }, children: [
                    "ENEMIES KILLED: ",
                    String(enemiesKilled).padStart(3, "0")
                  ] }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx(
                    "button",
                    {
                  ...
                  /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { color: GB.light, fontSize: 9, marginTop: 16 }, children: "PRESS SPACE TO RESTART" })
                ]
              }
            )
          }
...
function App() {
  return /* @__PURE__ */ jsxRuntimeExports.jsx(SkunkRunner, {});
}
...
const CategoryRequest = Record({
  "region": Text,
  "category": Text
});
const InflationSeries = Vec(Tuple(Nat, Float64));
Service({
  "getInflationData": Func([CategoryRequest], [InflationSeries], []),
  "getRegionalCategories": Func([Text], [Vec(Text)], []),
  "getTopInflationSubcategories": Func(
    [Text, Nat],
